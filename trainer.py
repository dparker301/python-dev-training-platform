...
def main():
    ...
    sp = sub.add_parser("open", help="Open a lab README")
    sp.add_argument("lab_id")
    sp2 = sub.add_parser("start", help="Mark started + open README")
    sp2.add_argument("lab_id")
    sp3 = sub.add_parser("complete", help="Mark a lab complete")
    sp3.add_argument("lab_id")
    args = ap.parse_args()
    if args.cmd == "list":
        list_items()
    elif args.cmd == "open":
        open_lab(args.lab_id)
    elif args.cmd == "start":
        start_lab(args.lab_id)
    elif args.cmd == "complete":
        complete_lab(args.lab_id)
    else:
        ap.print_help()
