...
def load_csv(path):
    with open(path) as f, get_conn() as conn:
        ...
        conn.executemany(
            "insert into employees values (:id,:name,:department,:salary)", rows
        )
        conn.commit()
        print(f"Loaded {len(rows)} rows into {DB.name}")
...
if __name__ == "__main__":
    ...
    line = sub.add_parser("load")
    line.add_argument("path")
